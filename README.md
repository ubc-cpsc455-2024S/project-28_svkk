...
5. Our application is currently being hosted through AWS EC2, an industry-standard platform known for its robustness and scalability in web services hosting which doesn't spin down despite inactivity, which is an improvement compared to Render. We made use of security groups to block requests from different ip-addresses and ports, enhancing the security of our application. Later, we integrated a Route 53 custom domain to give our application a user-friendly domain name, and used ELB and Amazon certificates to encrypt our server communication.
